...
function Partida:carregarCartas()
    ...
    -- Para modos cooperativo e solo, usa todas as 12 cartas para formar grupos
    local numTipos = 12  -- Usa todas as 12 imagens diferentes
    
    for i = 0, numTipos - 1 do
        local carta = Carta:new(i, Config.deck[i + 1])
        table.insert(cartas, carta)
    end
    
    print("Carregadas " .. #cartas .. " tipos de cartas (IDs 0 a " .. (numTipos-1) .. ")")
    return cartas
end
...
function Partida:verificaGrupoCartas()
    ...
    if grupoFormado then
        ...
        self.score = self.score + #self.cartasViradasNoTurno * 100
        self.tabuleiro:removerGrupoEncontrado(self.cartasViradasNoTurno)
        self.cartasViradasNoTurno = {}
        return true
    end
    return false
...
function Partida:finalizarPartida(vitoria)
    ...
    if vitoria then
        ...
        self.score = self.score + bonusTempo
